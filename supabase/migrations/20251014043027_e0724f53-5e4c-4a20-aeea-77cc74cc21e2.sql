-- Update admin_update_profile function to include city and state parameters
...
DECLARE
  ...
  col_city_exists boolean := false;
  col_state_exists boolean := false;
...
BEGIN
  ...
  IF p_city IS NOT NULL AND col_city_exists THEN
    set_parts := set_parts || format('city = %L', p_city);
  END IF;

  IF p_state IS NOT NULL AND col_state_exists THEN
    set_parts := set_parts || format('state = %L', p_state);
  END IF;
