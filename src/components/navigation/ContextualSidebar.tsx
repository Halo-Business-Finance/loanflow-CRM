...
export function ContextualSidebar() {
  ...
  return (
    <Sidebar className={cn("border-r bg-card/60 backdrop-blur h-full", collapsed ? "w-16" : "w-72")} collapsible="icon">
      <SidebarContent className="py-4 h-full">
        <SidebarGroup>
                ...
                <SidebarMenuItem key={item.url}>
                  <SidebarMenuButton asChild>
                    <Link
                      to={item.url}
                      className={cn(
                        "flex items-center gap-3 px-3 py-3 text-sm rounded-lg transition-all duration-200 font-medium",
                    ...
                    >
                      ...
                      {!collapsed && <span className="truncate leading-relaxed">{item.title}</span>}
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              ))}
